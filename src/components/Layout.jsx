...
const navLinks = [
  ...
  { path: '/news', label: 'News', mobileLabel: 'News & Events' },
...
export default function Layout({ children }) {
  ...
  const NavLink = ({ link, mobile = false }) => (
    <Link 
      ...
      className={`${mobile ? 'block' : ''} hover:text-blue-200 transition ${mobile ? 'text-sm lg:text-base' : ''} ${isActive(link.path) ? 'text-blue-200' + (mobile ? '' : ' border-b-2 border-blue-200') : ''}`}
    >
      {mobile && link.mobileLabel ? link.mobileLabel : link.label}
    </Link>
  )
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <ul className="hidden md:flex space-x-4 lg:space-x-6">
              {navLinks.map(link => <li key={link.path}><NavLink link={link} /></li>)}
            </ul>
          </div>
          
          {mobileMenuOpen && (
            <ul className="md:hidden flex flex-col space-y-3 mt-4 pb-4">
              {navLinks.map(link => <li key={link.path}><NavLink link={link} mobile /></li>)}
            </ul>
          )}
        </div>
      </nav>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            <div>
              ...
              <p className="text-sm md:text-base text-gray-300">Empowering students to achieve their full potential through quality education and innovative learning.</p>
            </div>
            <div>
              <h3 className="text-lg md:text-xl font-bold mb-4">Quick Links</h3>
              <ul className="space-y-2 text-sm md:text-base text-gray-300">
                {navLinks.map(link => (
                  <li key={link.path}><Link to={link.path} className="hover:text-white">{link.mobileLabel || link.label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
          ...
          <div className="border-t border-gray-700 mt-8 pt-4">
            <div className="flex justify-center gap-6 mb-4">
              <a href="https://www.instagram.com/denoblescholarsacademy?igsh=NDNwdGQ5N2RnOHVu&utm_source=qr" target="_blank" rel="noopener noreferrer" className="text-gray-300 hover:text-white transition" title="Instagram">
              ...
              <a href="https://www.tiktok.com/@_denoblescholarsacademy?_r=1&_t=ZS-93fdMMaH30H" target="_blank" rel="noopener noreferrer" className="text-gray-300 hover:text-white transition" title="TikTok">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
